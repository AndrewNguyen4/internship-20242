...
# Tokenize and get average embedding for a context sentence
def get_context_embedding(sentence, tokenizer, model):
    tokens = tokenizer.texts_to_sequences([sentence])[0]
    if not tokens:
        return None
    
    # Filter out OOV token indices
...
# Get embedding for a single candidate word
def get_word_embedding(word, tokenizer, model):
    idx = tokenizer.word_index.get(word)

    if idx is None or idx >= tokenizer.num_words:
        return None  # Word is OOV (out-of-vocab) for embedding
    embedding = model.get_layer("w2v_context")(tf.constant([idx]))
    return embedding


# Re-rank predictions based on N-gram score and embedding similarity
def rerank_predictions(sentence, predictions, model, tokenizer, lambda_weight=0.5):
    """
    Args:
        sentence (str): The context input sentence.
        predictions (list of tuples): [(candidate_word, ngram_score), ...]
        model_path (str): Path to the saved Word2Vec model.
        tokenizer_path (str): Path to the saved tokenizer.
        lambda_weight (float): Weight between n-gram score and embedding similarity.
    Returns:
        list of (word, combined_score) sorted by highest combined_score
    """
...
def test_word_prediction(test_file, ngram_model, num_of_preds=5,
                          model_path=None, tokenizer_path=None, lambda_weight=0.5):
    """Evaluates word prediction accuracy using an N-gram model with optional re-ranking."""
    ...
    with open(test_file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            prefix = prefix.split()
            predictions = ngram_model.predict_next_word_2(prefix, k=num_of_preds)
            ...
            if predictions:
                ...
                predicted_words = [word for word, _ in reranked_predictions]
            else:
            ...
            if random.random() < 0.0005:
                print(line, predictions, reranked_predictions)
            total += 1
...
if __name__ == "__main__":
    ngram_model_path = r"ngram\ngram_model_3.pkl"  
    test_prediction_file = r"mobiletext\sent_test_prediction.txt"  
    model_path = r'word2vec_model_finetuned.keras'
    tokenizer_path = r'C:\Users\AndrewNguyen\Desktop\CodingProject\20242\Internship\word2vec_model_tokenizer.pkl'
    num_of_preds = 3 
    lambda_weight = 0.7
